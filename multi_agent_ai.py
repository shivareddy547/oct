...
class OllamaAnalyzer:
    ...
    def _get_component_code(self, component_name: str, component_data: dict) -> str:
        ...
        - Only allows exact matches: component_name.tsx, component_name.ts, component_name.js
        - Preference order: .tsx > .ts > .js
        - If multiple files found across project roots, returns instruction to refer manually
        - Includes requirement explanation if code is missing
        """
...
install_commands:
  - "echo 'Error in YAML generation'"'''
    ...
    def apply_changes(self, yaml_response: str) -> Dict:
        """Apply changes from LLM YAML response to project files and run install commands."""
        try:
            ...
            print("====================================================")
            print("Initial Results Object:", results)
            print("====================================================")
            ...
            if install_commands:
                ...
                for command in install_commands:
                    print(f"⚙️ Running install command: {command}")
                    try:
                        ...
                        # Try to infer working directory from the command
                        if "cd " in command:
                            ...
                            if len(parts) > 1:
                                ...
                                if cd_part.startswith("cd "):
                                    project_dir = cd_part.replace("cd ", "").strip()
                                    process = subprocess.Popen(
                                        parts[1].strip(),
                                        shell=True,
                                        stdout=subprocess.PIPE,
                        ...
                        if process.returncode == 0:
                            results['install_output'].append(f"✅ Success: {command}\n{output}")
                            print(f"✅ Packages installed for command: {command}")
                        else:
                            results['install_output'].append(f"❌ Error running: {command}\n{error_output}")
                            print(f"❌ Failed to run: {command}")
                    except Exception as e:
                        results['install_output'].append(f"⚠️ Exception while running '{command}': {str(e)}")
                        print(f"⚠️ Exception while running: {command} -> {e}")

            # ✅ Update Redis cache with project structure
            ...
            <div class="input-area">
                <div class="model-selector">
                    ...
                    <select id="model-select">
                      ...
                      <option value="mahonzhan/all-MiniLM-L6-v2:latest">mahonzhan/all-MiniLM-L6-v2:latest</option>
                      ...
                      <option value="devstral:24b">devstral:24b</option>
                      <option value="deepseek-r1-14b-finetuned:latest">deepseek-r1-14b-finetuned:latest</option>
                      <option value="my-react-assistant:latest">my-react-assistant:latest</option>
                      <option value="qllama/bge-reranker-large:latest">qllama/bge-reranker-large:latest</option>
                      <option value="pedrovillalobos/mxbai-embed-large-no-gpu:latest">pedrovillalobos/mxbai-embed-large-no-gpu:latest</option>
                      <option value="phi:2.7b">phi:2.7b</option>
                      <option value="turingdance/gte-large-zh:latest">turingdance/gte-large-zh:latest</option>
                      <option value="bge-large:335m">bge-large:335m</option>
                      <option value="gemma:7b">gemma:7b</option>
                      <option value="mxbai-embed-large:latest">mxbai-embed-large:latest</option>
                      ...
                      <option value="codegeex4:latest">codegeex4:latest</option>
